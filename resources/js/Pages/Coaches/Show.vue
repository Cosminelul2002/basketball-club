            ...
            <div class="mx-auto max-w-7xl px-6 lg:px-8">
                ...
                <ul role="list"
                    ...
                    <li v-for="coach in coaches" :key="coach.id">
                        <img class="aspect-[3/2] w-full rounded-2xl object-cover" :src="coach.image" alt="" />
                        <h3 class="mt-6 text-lg font-semibold leading-8 tracking-tight text-gray-900">{{
                        coach.first_name }}
                            {{ coach.last_name }}</h3>
                        <p class="text-base leading-7 text-gray-600">{{ coach.date_of_birth }}</p>
    ...
    methods: {
        mapImage() {
            this.coaches.map(coach => {
                // coach.imageUrl = `https://ui-avatars.com/api/?name=${coach.first_name}+${coach.last_name}&size=300`;
                coach.image = '../images/coach.jpg';
            });
        }
    }
}
